 ...
 * ---- SPEED TO ANGLE PID -----
 * ----------------------------- */
const float kP_speed = -0.0008;
const float kI_speed = -0.001;
const float kD_speed = 0.0005 / 0.0035;
...
//const float kD_angle = 0.5 / 0.05;

float kP_angle = 40.0;
float kI_angle = 0.5;
float kD_angle = 0.2 / 0.004 ;
...
void loop() {
  ...
  if(Serial1.available() > 0) {
    if(Serial1.read() == 'P') {
      angleToMotorPIDController._kP = Serial1.parseFloat();
    }
    if(Serial1.read() == 'I') {
      angleToMotorPIDController._kI = Serial1.parseFloat();
    }
    if(Serial1.read() == 'D') {
      angleToMotorPIDController._kD = Serial1.parseFloat();
    }
  }
  ...
  // complementary filter
  pitch = COMPLEMENTARY_GAIN * (lastPitch + getGyroXRate() * loopTime / 1000) + (1 - COMPLEMENTARY_GAIN) * (getAccX() - zeroIMUAngle);
  lastPitch = pitch;
 
  Serial.println(getAccX());
  Serial1.print(" ");
