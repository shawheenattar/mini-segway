...
PIDController::PIDController(float kP, float kI, float kD, bool integralQueue, bool pGainScheduling, bool derivativeKick)
{
  // gains
  _kP = kP;
  _kI = kI;
...
float PIDController::compute(float currentReading, float setpoint)
{
  _error = currentReading - setpoint;
  return (pTerm(currentReading) + iTerm() + dTerm(currentReading));
}

float PIDController::feedforwardCompute(float currentReading, float setpoint, float kFF) {
  _feedback = compute(currentReading, setpoint);
  _feedforward = kFF * setpoint;
  return (_feedback + _feedforward);
}

float PIDController::pTerm(float currentReading)
{
  if (_pGainScheduling) {
...
float PIDController::dTerm(float currentReading)
{
